...
function ogm_qs_piece_from_pair(float $lengthIn, float $widthIn, string $kind): array
{
    $lengthIn = round($lengthIn, 2);
    $widthIn  = round($widthIn, 2);
    $sqFt     = round($lengthIn * $widthIn / 144, 2);
    if ($kind === 'splash') {
        return ['label' => 'Splash (text)', 'lengthIn' => $lengthIn, 'heightIn' => $widthIn, 'sqFt' => $sqFt, 'source' => 'text'];
    }
    return ['label' => 'Counter (text)', 'lengthIn' => $lengthIn, 'widthIn' => $widthIn, 'sqFt' => $sqFt, 'source' => 'text'];
}
...
function ogm_qs_parse_text_measurements(string $description): array
{
    ...
    foreach ($lines as $line) {
        ...
        foreach ($pairs as $pair) {
            $a = ogm_qs_parse_inches_token($pair[1]);
            $b = ogm_qs_parse_inches_token($pair[2]);
            if ($a === null || $b === null) continue;
            $lengthIn = max($a, $b);
            $widthIn  = min($a, $b);
            $isSplash = (bool)preg_match('/splash|backsplash|bspl/i', $line);
            if ($isSplash || ($widthIn <= 12 && $widthIn > 0)) {
                $rooms[$roomKey]['splashPieces'][] = ogm_qs_piece_from_pair($lengthIn, $widthIn, 'splash');
            } else {
                $rooms[$roomKey]['counterPieces'][] = ogm_qs_piece_from_pair($lengthIn, $widthIn, 'counter');
            }
        }
    }
