...
function ogm_qs_apply_room_hours(array &$aiRooms, ?float $jobHours, string $jobHoursConf): void
{
    ...
    $singleRoom = $n === 1;
    $splitJobHours = null;
    if (!$singleRoom && $jobHours !== null && $jobHours > 0 && in_array($jobHoursConf, ['high', 'medium'], true)) {
        $splitJobHours = round(($jobHours / $n) * 2) / 2;
    }
    ...
    foreach ($aiRooms as &$room) {
        ...
        if ($singleRoom && $jobHours !== null && $jobHours > 0) {
            $override = $jobHours;
            $conf = $jobHoursConf;
        } elseif ($splitJobHours !== null && !$roomHasExplicitHours) {
            $override = $splitJobHours;
            $conf = $jobHoursConf;
        }
        $resolved = ogm_qs_resolve_room_hours($room, (string)($room['label'] ?? ''), $override, $conf);
