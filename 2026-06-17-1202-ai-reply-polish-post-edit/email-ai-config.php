...
function ogm_ai_voice_and_rules(): string {
  return <<<PROMPT
ABOUT OGM:
...
function ogm_ai_system_prompt(): string {
  return "You are drafting an email reply on behalf of an Olive Glass & Marble (OGM) sales rep.\n\n" . ogm_ai_voice_and_rules();
}

function ogm_ai_rewrite_system_prompt(): string {
  return "You are rewriting a staff-written email reply for Olive Glass & Marble (OGM).\n\n"
    . "REWRITE TASK:\n"
    . "- Preserve every fact, number, date, name, commitment, and bracketed placeholder from the user's draft\n"
    . "- Do not add new prices, dates, availability, or scheduling promises\n"
    . "- Improve warmth, clarity, paragraph flow, and consistency with the customer's formality level\n"
    . "- Keep the rep's sign-off style (first name only)\n"
    . "- Return only the rewritten email body — no preamble, labels, or explanation\n\n"
    . ogm_ai_voice_and_rules();
}
...
function ogm_ai_call(string $userMessage, ?string $systemPrompt = null): array {
  ...
  $payload = [
    'model' => $model,
    'max_tokens' => 800,
    'system' => $systemPrompt !== null && $systemPrompt !== '' ? $systemPrompt : ogm_ai_system_prompt(),
...
Mark as junk: marketing, newsletters, cold sales pitches, SEO scams, unrelated vendor spam, obvious mass mail, and routine automated notifications from GoDaddy, ClickUp, GReminders/GoReminders (appointment/SMS opt-out alerts, reminders), and Webgility (task updates, comment alerts, appointment reminders, hosting notices, daily automation/sync summaries, etc.).
Always mark as keep: password resets, verification codes, MFA/2FA codes, identity verification, security alerts, real customers, contractors, job-related mail, quotes, and scheduling.
