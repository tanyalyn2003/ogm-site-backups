...
require_once __DIR__ . DIRECTORY_SEPARATOR . 'todo-lib.php';
...
function ogm_email_state_defaults(): array {
  return ['tags' => [], 'flags' => [], 'reminders' => [], 'attachmentLog' => [], 'jobNoteLog' => [], 'junkFilter' => false, 'junkCache' => [], 'junkAllow' => []];
}
...
function ogm_email_remove_message_state(string $username, array $messageIds): void {
  ...
  foreach (array_keys($stateData['flags'] ?? []) as $mid) {
    if (isset($idSet[$mid])) unset($stateData['flags'][$mid]);
  }
...
function ogm_email_ai_thread_context(string $token, string $username, string $messageId): array {
  ...
  if (($msg['_status'] ?? 0) !== 200) {
    return ['_error' => 'Could not load message for AI drafting.', '_status' => 502];
  }
  $fromAddr = strtolower((string)($msg['from']['emailAddress']['address'] ?? ''));
  $subject = (string)($msg['subject'] ?? '');
  if (ogm_email_is_automated_sender($fromAddr, $subject)) {
    return ['_error' => 'AI drafting is hidden for no-reply, bounce, and automated emails.', '_status' => 400];
  }
  $thread = [];
  ...
  if (trim($convoText) === '') {
    return ['_error' => 'This email does not have enough readable text for an AI draft.', '_status' => 400];
  }
  return [
...
function ogm_email_is_vendor_notification(array $msg): bool {
  ...
  if (strpos($blob, 'goreminders') !== false || strpos($blob, 'greminders') !== false || strpos($blob, 'go reminders') !== false
    || preg_match('/@(goreminders|greminders|go-reminders)\./i', $from)) {
    return true;
  }
  if (strpos($blob, 'webgility') !== false || preg_match('/@([a-z0-9.-]*\.)?webgility\.com$/i', $from)) {
    return true;
  }
...
/** Always hide vendor tool notifications from Inbox (independent of AI Filter toggle). */
function ogm_email_apply_vendor_junk_filter(string $username, array $messages): array {
  if (!$messages) {
  ...
  foreach ($messages as $msg) {
    ...
    if (ogm_email_should_hide_as_junk($msg, $stateData, false)) {
      $hidden++;
      continue;
    }
    $out[] = $msg;
...
function ogm_email_apply_junk_filter(string $username, array $messages, bool $aiFilterOn, bool $runAi = true): array {
  if (!$aiFilterOn || !$messages) {
    return ['messages' => $messages, 'hidden' => 0];
  }
  $stateData = ogm_email_load_state($username);
  $cache = is_array($stateData['junkCache'] ?? null) ? $stateData['junkCache'] : [];
  $needsAi = [];
  ...
  foreach ($messages as $msg) {
    ...
    if (ogm_email_should_hide_as_junk($msg, $stateData, $aiFilterOn)) {
      $hidden++;
      continue;
    }
    $out[] = $msg;
...
function ogm_email_public_reminder(array $r): array {
  return [
    ...
    'note' => (string)($r['note'] ?? ''),
    'done' => !empty($r['done']),
    'todoItemId' => (string)($r['todoItemId'] ?? ''),
...
function ogm_email_uploaded_attachments(int $maxTotalBytes = 20971520): array {
  ...
  foreach ($names as $i => $rawName) {
    ...
    $out[] = [
      ...
      'contentType' => ogm_email_guess_mime_type((string)$rawName, trim((string)($types[$i] ?? ''))),
      'size' => strlen($bytes),
      'bytes' => $bytes,
    ];
  }
...
function ogm_email_guess_mime_type(string $filename, string $browserType = ''): string {
  ...
  return match ($ext) {
    ...
    'txt' => 'text/plain',
    'csv' => 'text/csv',
    default => $browserType !== '' ? $browserType : 'application/octet-stream',
  };
}
...
function ogm_email_graph_reply_with_attachments(string $token, string $messageId, string $html, bool $replyAll, array $attachments): void {
  ...
  $createBody = $comment !== '' ? ['comment' => $comment] : [];
  $draft = ogm_graph_call($token, 'POST', '/me/messages/' . rawurlencode($messageId) . $draftEndpoint, $createBody ?: null);
  ...
  // Attach before send; avoid PATCHing the full reply body (breaks draft send on many mailboxes).
  ogm_email_graph_attach_files($token, $draftId, $attachments);

  $ready = ogm_graph_call($token, 'GET', '/me/messages/' . rawurlencode($draftId) . '?$select=isDraft,toRecipients,ccRecipients');
  if (($ready['_status'] ?? 0) === 200) {
    if (empty($ready['isDraft'])) {
      ogm_email_bad('Reply draft was not saved correctly. Try again.', 502);
    }
    if (empty($ready['toRecipients']) && empty($ready['ccRecipients'])) {
      ogm_email_bad('Reply draft has no recipients. Try Reply All or compose a new email.', 400);
    }
  }
...
function ogm_email_ai_quickstart_configured(): bool {
  if (!defined('OGM_QUICKSTART')) define('OGM_QUICKSTART', true);
  require_once __DIR__ . DIRECTORY_SEPARATOR . 'ai-quickstart-config.php';
  return ogm_qs_configured();
}

function ogm_email_build_quickstart_description(array $msg): string {
  $fromName = trim((string)($msg['from']['emailAddress']['name'] ?? $msg['from'] ?? ''));
  $fromAddr = trim((string)($msg['from']['emailAddress']['address'] ?? $msg['fromAddr'] ?? ''));
  if ($fromName !== '' && $fromAddr !== '' && stripos($fromName, $fromAddr) === false) {
    $fromLine = $fromName . ' <' . $fromAddr . '>';
  } else {
    $fromLine = $fromAddr !== '' ? $fromAddr : $fromName;
  }
  $subject = trim((string)($msg['subject'] ?? '(no subject)'));
  $dateRaw = (string)($msg['receivedDateTime'] ?? $msg['sentDateTime'] ?? $msg['date'] ?? '');
  $dateLine = $dateRaw;
  $ts = strtotime($dateRaw);
  if ($ts) $dateLine = date('D, M j, Y g:i A', $ts);
  $body = ogm_email_clean_text((string)($msg['body']['content'] ?? $msg['body'] ?? ''), 12000);
...
function ogm_email_next_pass_empty(): array {
  return [
    'customer' => ['linked' => false, 'customerId' => '', 'customerName' => '', 'entryId' => '', 'url' => ''],
    'lead' => ['linked' => false, 'leadId' => '', 'label' => '', 'url' => ''],
    'jobNote' => ['linked' => false, 'taskId' => '', 'taskName' => '', 'url' => ''],
  ];
}

function ogm_email_log_job_note(string $username, string $messageId, string $taskId, string $taskName = ''): void {
  $messageId = trim($messageId);
  $taskId = trim($taskId);
  if ($messageId === '' || $taskId === '') return;
  $state = ogm_email_load_state($username);
  $log = is_array($state['jobNoteLog'] ?? null) ? $state['jobNoteLog'] : [];
  $found = false;
  foreach ($log as &$row) {
    if (($row['messageId'] ?? '') === $messageId && ($row['taskId'] ?? '') === $taskId) {
      $row['taskName'] = $taskName !== '' ? $taskName : (string)($row['taskName'] ?? '');
      $row['savedAt'] = date('c');
      $found = true;
      break;
    }
  ...
  $state['jobNoteLog'] = $log;
  ogm_email_save_state($username, $state);
}

function ogm_email_next_pass_status(string $messageId, string $username): array {
  $messageId = trim($messageId);
  $out = ogm_email_next_pass_empty();
  if ($messageId === '') return $out;

  $tokensNow = ogm_graph_load_tokens($username);
  $mailbox = strtolower(trim((string)($tokensNow['mailbox'] ?? '')));

  $historyDir = ogm_email_customer_history_dir();
  foreach ((glob($historyDir . DIRECTORY_SEPARATOR . '*.json') ?: []) as $file) {
    $history = json_decode((string)@file_get_contents($file), true);
    if (!is_array($history) || !is_array($history['entries'] ?? null)) continue;
    $customerId = (string)($history['customerId'] ?? pathinfo($file, PATHINFO_FILENAME));
    foreach ($history['entries'] as $entry) {
      if (!is_array($entry)) continue;
      if ((string)($entry['messageId'] ?? '') !== $messageId) continue;
      if ($mailbox !== '' && strtolower((string)($entry['mailboxUser'] ?? '')) !== $mailbox) continue;

      if (empty($out['customer']['linked'])) {
        $customer = [];
        $custPath = ogm_email_customer_path($customerId);
        if (is_file($custPath)) {
          $decoded = json_decode((string)@file_get_contents($custPath), true);
          if (is_array($decoded)) $customer = $decoded;
        }
        $name = ogm_email_display_name($customer);
        if ($name === '') {
          $name = trim((string)($entry['from']['name'] ?? ''));
        }
        if ($name === '') $name = $customerId;
        $entryId = (string)($entry['id'] ?? '');
        $url = 'customer-db.php?cid=' . rawurlencode($customerId);
        if ($entryId !== '') $url .= '&emailEntry=' . rawurlencode($entryId);
        $out['customer'] = [
          'linked' => true,
      ...
      $jobId = trim((string)($entry['jobId'] ?? ''));
      if ($jobId !== '' && empty($out['jobNote']['linked'])) {
        $taskName = trim((string)($entry['jobName'] ?? ''));
        if ($taskName === '') $taskName = 'Job';
        $out['jobNote'] = [
          'linked' => true,
          'taskId' => $jobId,
          'taskName' => $taskName,
          'url' => 'job-tracking.php?task=' . rawurlencode($jobId),
        ];
      }
    }
  }

  $leadsPath = ogm_email_leads_path();
  if (is_file($leadsPath)) {
    $leads = json_decode((string)@file_get_contents($leadsPath), true);
    if (is_array($leads)) {
      foreach ($leads as $lead) {
        if (!is_array($lead)) continue;
        if ((string)($lead['messageId'] ?? '') !== $messageId) continue;
        $label = trim((string)($lead['fromName'] ?? ''));
        if ($label === '') $label = trim((string)($lead['subject'] ?? ''));
        if ($label === '') $label = 'Email lead';
        $leadId = (string)($lead['id'] ?? '');
        $out['lead'] = [
          'linked' => true,
          'leadId' => $leadId,
          'label' => $label,
          'url' => 'email-center.php?leadId=' . rawurlencode($leadId),
        ];
        break;
      }
    }
  ...
  if (empty($out['jobNote']['linked'])) {
    $state = ogm_email_load_state($username);
    $log = is_array($state['jobNoteLog'] ?? null) ? $state['jobNoteLog'] : [];
    foreach (array_reverse($log) as $row) {
      if (!is_array($row)) continue;
      if ((string)($row['messageId'] ?? '') !== $messageId) continue;
      ...
      $out['jobNote'] = [
        ...
        'taskId' => $taskId,
        'taskName' => $taskName,
        'url' => 'job-tracking.php?task=' . rawurlencode($taskId),
      ];
      break;
    }
  }
...
function ogm_email_find_lead_by_id(string $leadId): array {
  $leadId = trim($leadId);
  if ($leadId === '') return [];
  ...
  foreach ($leads as $lead) {
    if (is_array($lead) && (string)($lead['id'] ?? '') === $leadId) return $lead;
  }
  return [];
}
...
if ($action === 'status') {
  ...
  ogm_email_out([
    ...
    'canManageApiKeys' => qtCan('api_access'),
    ...
    'canUseAiQuickstart' => qtCan('ai_quickstart'),
    'aiQuickstartConfigured' => ogm_email_ai_quickstart_configured(),
  ]);
}
...
switch ($action) {
  case 'inbox':
    ...
    if ($folder === 'inbox' && $search === '') {
      $vendorFiltered = ogm_email_apply_vendor_junk_filter($username, $messages);
      $messages = $vendorFiltered['messages'];
      $hiddenJunk = (int)($vendorFiltered['hidden'] ?? 0);
      if ($aiFilter) {
        $aiFiltered = ogm_email_apply_junk_filter($username, $messages, true, true);
        $messages = $aiFiltered['messages'];
        $hiddenJunk += (int)($aiFiltered['hidden'] ?? 0);
      }
    }
  ...
  case 'message':
    ...
    $message = [
      ...
      'flag' => (string)($stateData['flags'][$id] ?? ''),
    ];
    ogm_email_out(['ok' => true, 'message' => $message, 'context' => ogm_email_message_context($message), 'nextPass' => ogm_email_next_pass_status($id, $username)]);
  ...
  case 'flag-list':
    $stateData = ogm_email_load_state($username);
    ogm_email_out(['ok' => true, 'flags' => $stateData['flags'] ?? []]);

  case 'flag-set':
    $messageId = trim((string)($input['messageId'] ?? ''));
    $flag = trim((string)($input['flag'] ?? ''));
    if ($messageId === '') ogm_email_bad('Missing message id.');
    $allowedFlags = ['', 'red', 'orange', 'green'];
    if (!in_array($flag, $allowedFlags, true)) ogm_email_bad('Unknown flag.');
    $stateData = ogm_email_load_state($username);
    if ($flag === '') unset($stateData['flags'][$messageId]); else $stateData['flags'][$messageId] = $flag;
    ogm_email_save_state($username, $stateData);
    ogm_email_out(['ok' => true, 'flag' => $flag]);
  ...
  case 'reminder-set':
    ...
    [$todoItemId] = ogm_todo_upsert_email_reminder($username, qtCurrentUser(), $reminder);
    if ($todoItemId !== '') $reminder['todoItemId'] = $todoItemId;
  ...
  case 'reminder-done':
    ...
    unset($r);
    ogm_email_save_state($username, $stateData);
    if (is_array($updatedReminder)) ogm_todo_upsert_email_reminder($username, qtCurrentUser(), $updatedReminder);
    ogm_email_out(['ok' => true]);
  ...
  case 'save-email-history':
    ...
    ogm_email_out(['ok' => true, 'entry' => $entry, 'nextPass' => ogm_email_next_pass_status($messageId, $username)]);
  ...
  case 'create-customer-from-email':
    ...
    ogm_email_out(['ok' => true, 'created' => !empty($result['created']), 'customer' => $result['customer'], 'entry' => $entry, 'nextPass' => ogm_email_next_pass_status($messageId, $username)]);
  ...
  case 'create-lead-from-email':
    ...
    @chmod($path, 0600);
    ogm_email_out(['ok' => true, 'lead' => $lead, 'created' => !$already, 'nextPass' => ogm_email_next_pass_status($messageId, $username)]);
  ...
  case 'save-email-job-note':
    ...
    ogm_email_log_job_note($username, (string)($msg['id'] ?? $messageId), $taskId, $taskName);
    ogm_email_out(['ok' => true, 'comment' => $commentRes, 'entry' => $entry, 'nextPass' => ogm_email_next_pass_status($messageId, $username)]);

  case 'email-quickstart-payload':
    if (!qtCan('ai_quickstart')) ogm_email_bad('AI Quick Start access required.', 403);
    if (!ogm_email_ai_quickstart_configured()) ogm_email_bad('AI Quick Start is not configured on the server.', 503);
    $messageId = trim((string)($_GET['messageId'] ?? $_GET['id'] ?? ''));
    if ($messageId === '') ogm_email_bad('Missing message id.');
    $msg = ogm_email_graph_message_for_archive($token, $messageId);
    $description = ogm_email_build_quickstart_description($msg);
    $normMessage = [
      ...
      'fromAddr' => (string)($msg['from']['emailAddress']['address'] ?? ''),
      'to' => array_map(static fn($t) => $t['emailAddress']['address'] ?? '', $msg['toRecipients'] ?? []),
      'cc' => array_map(static fn($t) => $t['emailAddress']['address'] ?? '', $msg['ccRecipients'] ?? []),
      'body' => (string)($msg['body']['content'] ?? ''),
    ];
    $ctx = ogm_email_message_context($normMessage);
    $cust = null;
    if (is_array($ctx['customers'] ?? null) && ($ctx['customers'][0] ?? null)) {
  ...
  case 'email-lead':
    $leadId = trim((string)($_GET['leadId'] ?? $_GET['id'] ?? ''));
    if ($leadId === '') ogm_email_bad('Missing lead id.');
    $lead = ogm_email_find_lead_by_id($leadId);
    if (!$lead) ogm_email_bad('Lead not found.', 404);
    ogm_email_out(['ok' => true, 'lead' => $lead]);
  ...
  case 'attachment-download':
    ...
    $guessMime = ogm_email_guess_mime_type($name, trim((string) ($_GET['contentType'] ?? '')));
    $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
    header_remove('Content-Type');
    if (ogm_heic_is_extension($ext) || ogm_heic_is_mime($guessMime)) {
      if (ogm_heic_serve_bytes_inline($bytes, $name)) exit;
    }
    if ($guessMime !== '' && str_starts_with($guessMime, 'image/')) {
      header('Content-Type: ' . $guessMime);
  ...
  case 'save-attachment-to-customer':
    ...
    $guessMime = ogm_email_guess_mime_type($name, $contentType);
    $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
    if (ogm_heic_is_extension($ext) || ogm_heic_is_mime($guessMime) || ogm_heic_is_mime($contentType)) {
        $jpeg = ogm_heic_bytes_to_jpeg($bytes);
        if ($jpeg !== null && $jpeg !== '') {
            $bytes = $jpeg;
            $size = strlen($bytes);
            $name = ogm_heic_jpeg_filename($name);
    ...
    $manifest[] = [
        ...
        'originalName'       => $name,
        'size'               => $size,
        'contentType'        => $contentType !== '' ? $contentType : $guessMime,
        'savedAt'            => date('c'),
        'savedBy'            => qtCurrentUser(),
  ...
  case 'ai-draft-reply':
    ...
    $messageId = trim((string)($input['messageId'] ?? ''));
    if ($messageId === '') ogm_email_bad('Missing message id.');
    $ctx = ogm_email_ai_thread_context($token, $username, $messageId);
    if (!empty($ctx['_error'])) ogm_email_bad((string)$ctx['_error'], (int)($ctx['_status'] ?? 502));
    $customerName = (string)($ctx['customerName'] ?? '');
    $customerAddr = (string)($ctx['customerAddr'] ?? '');
    $repName = (string)($ctx['repName'] ?? 'Tanya');
    $mailbox = (string)($ctx['mailbox'] ?? '');
    $convoText = (string)($ctx['convoText'] ?? '');
  ...
  case 'ai-rewrite-reply':
    ...
    $messageId = trim((string)($input['messageId'] ?? ''));
    if ($messageId === '') ogm_email_bad('Missing message id.');
    $draftText = trim((string)($input['draftText'] ?? ''));
    if (strlen($draftText) < 15) ogm_email_bad('Type at least a short reply before polishing.');
    $ctx = ogm_email_ai_thread_context($token, $username, $messageId);
    if (!empty($ctx['_error'])) ogm_email_bad((string)$ctx['_error'], (int)($ctx['_status'] ?? 502));
    $customerName = (string)($ctx['customerName'] ?? '');
    $customerAddr = (string)($ctx['customerAddr'] ?? '');
    ...
    $prompt = "CUSTOMER: " . ($customerName ?: $customerAddr ?: 'Unknown') . "\n"
      ...
      . "CONVERSATION HISTORY, OLDEST FIRST:\n\n"
      . $convoText
      . "-----\n\nUSER'S DRAFT REPLY (rewrite this — keep their facts and intent):\n\n"
      . $draftText
      . "\n\nRewrite the user's draft for OGM tone and clarity. Sign off with just '" . $repName . "'.";
    $draft = ogm_ai_call($prompt, ogm_ai_rewrite_system_prompt());
    if (!empty($draft['_error'])) {
      ogm_email_bad('AI polishing unavailable: ' . (string)$draft['_error'], (int)($draft['_status'] ?? 502));
    }
    ogm_ai_log_call($username, $mailbox, (int)($draft['inTokens'] ?? 0), (int)($draft['outTokens'] ?? 0), (string)($draft['model'] ?? ogm_ai_model()));
    ogm_email_out([
      'ok' => true,
