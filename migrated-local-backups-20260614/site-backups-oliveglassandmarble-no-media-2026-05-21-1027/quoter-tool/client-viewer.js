...
function drawPart2D(ctx, part, pad, scale){
  ...
  const partColor = part.partType === 'backsplash' ? '#a7f3d0' : '#93c5fd';
  ...
  if (ok) {
    ctx.globalAlpha = part.partType === 'backsplash' ? 0.18 : 0.16;
    ctx.fill();
    ctx.globalAlpha = 0.9;
    ctx.stroke();
  }
...
function build3D(snapshot){
  ...
  if (assemblyParts.length || assemblyBacksplashParts.length || assemblySplashCount) {
        ...
        .forEach((p) => {
          ...
          if (!topMat) {
            topMat = new THREE.MeshStandardMaterial({
              color: isBacksplash ? 0xa7f3d0 : 0x93c5fd,
              roughness: 0.55,
              metalness: 0.02
            });
          }
          const sideMat = new THREE.MeshStandardMaterial({
            color: isBacksplash ? 0x5b8b7a : 0x52525b,
            roughness: 0.88,
            metalness: 0.06,
            transparent: false,
            opacity: 1,
          ...
          if (isBacksplash) {
            ...
            mesh.position.set(xWorld, thickness + 0.06 + splashH / 2, zWorld);
