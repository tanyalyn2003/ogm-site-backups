...
function renderAssembled2D(snapshot){
  ...
  const counterParts = parts.filter((p) => p && p.partType === 'countertop' && p.assemblyPlacement && p.geom);
  ...
  const roomOffsets = computeAssemblyRoomOffsets(counterParts, {collapseToSingleOrigin: true});
  const flatItems = [];

  counterParts.forEach((part) => {
    const roomOffset = roomOffsets.get(String(part.assemblyPlacement?.roomId ?? part.roomId ?? 'room')) || {x:0, y:0};
    flatItems.push({
      type: 'countertop',
      part,
      geom: part.geom,
      ...
      placement: {
        x: (Number(roomOffset.x) || 0) + (Number(part.assemblyPlacement?.x) || 0),
        y: (Number(roomOffset.y) || 0) + (Number(part.assemblyPlacement?.y) || 0),
        rot: normDeg(part.assemblyPlacement?.rot || 0)
      },
  ...
  splashes.forEach((splash) => {
    ...
    if (!hostPart || !hostPart.assemblyPlacement || !hostPart.geom) return;
    const roomOffset = roomOffsets.get(String(hostPart.assemblyPlacement?.roomId ?? hostPart.roomId ?? 'room')) || {x:0, y:0};
    const edge = getAssemblyWorldEdge(hostPart, parseInt(splash.edgeIndex, 10) || 0, roomOffset);
    ...
    const ux = (edge.end.x - edge.start.x) / edge.len;
    const uy = (edge.end.y - edge.start.y) / edge.len;
    const midT = offset + len / 2 + userOffsetIn;
    const mid = {x: edge.start.x + ux * midT, y: edge.start.y + uy * midT};
    const edgeLocal = geomEdgeSegments(hostPart.geom).find((e) => e.index === (parseInt(splash.edgeIndex, 10) || 0));
    const inwardLoc = edgeLocal ? localEdgeInteriorNormal(hostPart.geom, edgeLocal) : {x:0, y:1};
    const inward = rotatePoint2D(inwardLoc.x, inwardLoc.y, normDeg(hostPart.assemblyPlacement?.rot || 0));
    ...
    flatItems.push({
      ...
      label: 'Backsplash',
      placement: {
        x: mid.x + inward.x * (height / 2),
        y: mid.y + inward.y * (height / 2),
  ...
  waterfalls.forEach((waterfall) => {
    ...
    if (!hostPart || !hostPart.assemblyPlacement || !hostPart.geom) return;
    const roomOffset = roomOffsets.get(String(hostPart.assemblyPlacement?.roomId ?? hostPart.roomId ?? 'room')) || {x:0, y:0};
    const edge = getAssemblyWorldEdge(hostPart, parseInt(waterfall.edgeIndex, 10) || 0, roomOffset);
    if (!edge || edge.len <= 0) return;
    ...
    const midT = offset + len / 2 + userOffsetIn;
    const mid = {x: edge.start.x + ux * midT, y: edge.start.y + uy * midT};
    const edgeLocal = geomEdgeSegments(hostPart.geom).find((e) => e.index === (parseInt(waterfall.edgeIndex, 10) || 0));
    const inwardLoc = edgeLocal ? localEdgeInteriorNormal(hostPart.geom, edgeLocal) : {x:0, y:1};
    const inward = rotatePoint2D(inwardLoc.x, inwardLoc.y, normDeg(hostPart.assemblyPlacement?.rot || 0));
  ...
  const bounds = assembled2DBounds(flatItems);
...
function drawAssembledItem2D(ctx, item, stonesByKey, opts){
  ...
  if (item.type === 'countertop') drawAssembledCutouts2D(ctx, item.cutouts, scale);
  ctx.restore();
...
function drawAssembledTextureFill(ctx, geom, part, stone, imgUrl, scale){
  ...
  const patternW = Math.max(1, Math.round(slabW * scale));
  const patternH = Math.max(1, Math.round(slabH * scale));
  patternCanvas.width = patternW;
  patternCanvas.height = patternH;
  const pctx = patternCanvas.getContext('2d');
  pctx.drawImage(cached, 0, 0, patternW, patternH);
  ...
  const d = -s;
  const e = c;
  const tx = ((-center.x * c) + (center.y * s) + (slabW / 2)) * scale;
  const ty = ((-center.x * s) - (center.y * c) + (slabH / 2)) * scale;
  pattern.setTransform(new DOMMatrix([a, b, d, e, tx, ty]));
