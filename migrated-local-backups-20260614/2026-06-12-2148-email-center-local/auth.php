...
function qtAppBasePath() {
	$scriptName = (string) ($_SERVER['SCRIPT_NAME'] ?? '/');
	$scriptName = str_replace('\\', '/', $scriptName);
	$marker = '/quoter-tool/';
	$pos = strpos($scriptName, $marker);
	if ($pos !== false) {
		return substr($scriptName, 0, $pos + strlen('/quoter-tool'));
	}
	$bp = qtBasePath();
	$bp = rtrim($bp, '/');
	if (str_ends_with($bp, '/user-admin')) {
		$bp = rtrim(dirname($bp), '/');
	}
	return $bp === '' ? '' : $bp;
}
...
function qtStartSession() {
	...
	$appCookiePath = qtSessionCookiePath();
	$legacyAdminCookiePath = rtrim($appCookiePath, '/') . '/user-admin/';
	if ($legacyAdminCookiePath !== $appCookiePath) {
		setcookie(session_name(), '', [
			'expires' => time() - 42000,
			'path' => $legacyAdminCookiePath,
			'domain' => '',
...
require_once __DIR__ . DIRECTORY_SEPARATOR . 'quoter-users.php';

function qtSetSessionUser($user) {
	qtStartSession();
	session_regenerate_id(true);
	$_SESSION['qt_logged_in'] = true;
	$_SESSION['qt_username'] = (string) ($user['username'] ?? '');
	$_SESSION['qt_display_name'] = trim((string) ($user['display_name'] ?? $_SESSION['qt_username']));
	$_SESSION['qt_role'] = qtNormalizeRole($user['role'] ?? '');
}
...
function qtAttemptLogin($username, $password) {
	...
	$authUser = qtAuthenticateUser($username, $password);
	if (is_array($authUser)) {
		qtSetSessionUser($authUser);
		return [true, ''];
	}

	// Legacy fallback while auth-config still exists (remove after full cutover).
	$hash = qtPasswordHash();
	...
	qtSetSessionUser([
		'username' => $username !== '' ? $username : qtUsername(),
		'display_name' => $username !== '' ? $username : qtUsername(),
