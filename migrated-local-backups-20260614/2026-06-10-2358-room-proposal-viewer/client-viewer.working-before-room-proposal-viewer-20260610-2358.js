...
function groupCounterPartsByRoom(parts){
  if (!viewerSeparateRooms) return [{key:'all', label:'All Rooms', parts: parts || []}];
  const groups = [];
...
function renderAssembled2D(snapshot){
  ...
  groupCounterPartsByRoom(counterParts).forEach((group, groupIndex) => {
    if (viewerSeparateRooms) {
      if (groupIndex > 0) {
        cursorX = 0;
...
function draw2DRoomSectionLabels(ctx, sections, opts){
  ...
  sections.forEach((section) => {
    ...
    ctx.fill();
    ctx.strokeStyle = 'rgba(196,160,90,.45)';
    ctx.lineWidth = 1;
    ctx.stroke();
...
function drawCenteredPieceLabel2D(ctx, geom, label, scale, type){
  ...
  ctx.fillStyle = type === 'countertop' ? '#f8fafc' : '#e2e8f0';
...
function drawAssembledDimensions2D(ctx, item, opts){
  ...
  edges.forEach((edge) => {
    ...
    const box = {x: labelX - tw / 2 - 6, y: labelY - 8, w: tw + 12, h: 16, key};
    if ((state.occupied || []).some((other) => rectsOverlap2D(box, other))) return;

    ctx.save();
    ctx.strokeStyle = type === 'countertop' ? 'rgba(196,160,90,.82)' : 'rgba(148,163,184,.68)';
    ...
    ctx.lineWidth = 1;
    roundRectPath2D(ctx, box.x, box.y, box.w, box.h, 4);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = type === 'countertop' ? '#fcd34d' : '#cbd5e1';
...
function rerender2DViewer(){
  if (typeof render2D === 'function' && SNAPSHOT) render2D(SNAPSHOT);
}
