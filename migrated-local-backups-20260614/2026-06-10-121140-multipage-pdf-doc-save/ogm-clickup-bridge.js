...
(function () {
  ...
  async function createProjectProposalPackageForJobTracking(productLabel) {
    if (typeof window.ogmCreateProjectProposalPackage !== 'function') return null;
    try {
      showBridgeToast('Saving project proposal package...');
      var proposal = await window.ogmCreateProjectProposalPackage();
      if (proposal && proposal.viewUrl) {
        showBridgeToast('Project proposal package saved');
      }
      return proposal || null;
    } catch (err) {
      console.warn('[OGM Bridge] project proposal package failed', err);
      showBridgeToast((productLabel || 'Quote') + ' project proposal did not save. You can regenerate it from Job Tracking.', true);
      return null;
    }
  }
  ...
  window.sendToClickUp = async function sendToClickUp() {
    ...
    projectProposal = await createProjectProposalPackageForJobTracking('Stone');
    ...
    openJobTrackingWithPayload({
      ...
      projectProposal: projectProposal ? {
        ...
        version: projectProposal.version || 1,
        viewUrl: projectProposal.viewUrl || '',
        quoteNumber: projectProposal.quoteNumber || quoteNumber || '',
        productLine: projectProposal.productLine || 'stone'
      } : null
    }, jobWin);
  ...
  window.sendGlassToClickUp = async function sendGlassToClickUp() {
    ...
    try {
      showBridgeToast('Saving quote before Job Tracking...');
      await saveQuoteBeforeJobTracking();
      quoteNumber = getQuoteNumber() || quoteNumber;
      customerId = getCustomerId() || customerId;
    } catch (err) {
      console.warn('[OGM Bridge] glass save before Job Tracking failed', err);
      showBridgeToast('Opening Job Tracking — Glass quote save may need checking');
    }

    projectProposal = await createProjectProposalPackageForJobTracking('Glass');
    ...
    openJobTrackingWithPayload({
      ...
      projectProposal: projectProposal ? {
        ...
        viewUrl: projectProposal.viewUrl || '',
        quoteNumber: projectProposal.quoteNumber || quoteNumber || '',
        productLine: projectProposal.productLine || 'glass'
      } : null
    });
