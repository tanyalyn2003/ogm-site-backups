...
function customersApiApplyQuoteToCustomer(array &$cust, array $quote) {
    ...
    $savedAt = trim((string) ($quote['savedAt'] ?? ''));
    if ($savedAt === '') {
        $savedAt = gmdate('c');
    }
    $entry = [
...
if ($action === 'patch-glass-work') {
    ...
    $allowed = [
        'name', 'job', 'salesperson', 'rep', 'sqft', 'totalSF',
        'invoiceNumber', 'invoiceTotal', 'invoiceDate', 'installDate',
        'hrsAct', 'hrsEst', 'glassWorkHours', 'stoneWorkHours', 'cosTotal', 'jobCode', 'jobType',
        'taxMode', 'taxBackoutAmount', 'taxBase', 'taxExemptReason', 'useNcRpSplit', 'cosTax',
        'depositReceiptNumber', 'depositAmount', 'depositRecordedAt',
    ];
    foreach ($allowed as $key) {
        if (array_key_exists($key, $patch)) {
            $summary[$key] = $patch[$key];
...
// Body: { "customerId"?: "C…", "fields": { … }, "quote"?: { invoiceNum, quoteNumber, … }, "source"?: "…" }
// Finds by linked id, else phone/name; merges contact fields and optional quote row on customer.quotes[].
// ────────────────────────────────────────────────────────────────────────────
if ($action === 'upsert-from-quote') {
    ...
    $quote = isset($body['quote']) && is_array($body['quote']) ? $body['quote'] : null;
    if ($quote !== null) {
        customersApiApplyQuoteToCustomer($cust, $quote);
    }
    $cust['id'] = $targetId;
    $cust['updatedAt'] = gmdate('Y-m-d');
