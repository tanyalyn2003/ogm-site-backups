...
require_once dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'lead-storage.php';
...
function sopStorageDir() {
  return dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . 'ogm-sop';
}
...
function sopCurrentUser() {
  ...
  $username = sopNormalizeUsername($_SESSION['qt_username'] ?? $displayName);
  if ($username === '') $username = 'staff';

  return [
    'username' => $username,
    'display_name' => trim((string) $displayName) ?: 'Staff',
    'role' => 'manager',
  ];
}
