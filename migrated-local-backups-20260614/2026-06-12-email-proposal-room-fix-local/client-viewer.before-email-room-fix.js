...
function viewer2DPalette(){
  if (!viewerIsLightTheme) {
    return {
      canvasBg: '#0b1222',
      countertopFill: 'rgba(147,197,253,.22)',
      backsplashFill: 'rgba(167,243,208,.22)',
      waterfallFill: 'rgba(56,189,248,.22)',
      countertopStroke: '#93c5fd',
      backsplashStroke: '#a7f3d0',
      waterfallStroke: '#38bdf8',
      labelStroke: 'rgba(2,6,23,.82)',
      labelFillCounter: '#f8fafc',
      labelFillOther: '#e2e8f0',
      roomBadgeBg: 'rgba(2,6,23,.82)',
      roomBadgeStroke: 'rgba(196,160,90,.45)',
      roomBadgeText: '#fcd34d',
      dimensionLineCounter: 'rgba(196,160,90,.82)',
      dimensionLineOther: 'rgba(148,163,184,.68)',
      dimensionBoxBg: 'rgba(2,6,23,.78)',
      dimensionBoxStroke: 'rgba(148,163,184,.34)',
      dimensionTextCounter: '#fcd34d',
      dimensionTextOther: '#cbd5e1',
      cutoutStroke: '#e2e8f0',
      seamStroke: '#f59e0b'
    };
  }
  return {
    canvasBg: '#ffffff',
    countertopFill: 'rgba(30,64,175,.10)',
    backsplashFill: 'rgba(22,101,52,.12)',
    waterfallFill: 'rgba(30,64,175,.10)',
    countertopStroke: '#1e3a8a',
    backsplashStroke: '#166534',
    waterfallStroke: '#1e3a8a',
    labelStroke: 'rgba(255,255,255,.88)',
    labelFillCounter: '#111827',
    labelFillOther: '#1f2937',
    roomBadgeBg: 'rgba(255,255,255,.92)',
    roomBadgeStroke: 'rgba(127,29,29,.45)',
    roomBadgeText: '#7f1d1d',
    dimensionLineCounter: 'rgba(127,29,29,.88)',
    dimensionLineOther: 'rgba(30,58,138,.74)',
    dimensionBoxBg: 'rgba(255,255,255,.94)',
    dimensionBoxStroke: 'rgba(30,58,138,.38)',
    dimensionTextCounter: '#7f1d1d',
    dimensionTextOther: '#1e3a8a',
    cutoutStroke: '#1e3a8a',
    seamStroke: '#7f1d1d'
  };
}
...
function filterViewerSnapshotByRoom(snapshot, roomKey){
  ...
  parts.forEach((part) => {
    if (!part || part.partType !== 'countertop') return;
    hostRoomById.set(String(part.id), viewerRoomKeyForPart(part));
  });
  ...
  parts.forEach((part) => {
    ...
    if (!id) return;
    if (viewerRoomKeyMatches(viewerRoomKeyForPart(part), roomKey)) {
      keepIds.add(id);
      return;
    }
    if ((isBacksplashPart(part) || isWaterfallPart(part)) && viewerRoomKeyMatches(viewerRoomKeyForPart(part), roomKey)) {
      keepIds.add(id);
    }
  });
  ...
  const splashes = Array.isArray(assembly?.splashes) ? assembly.splashes.filter((s) => {
    ...
    return viewerRoomKeyMatches(hostRoomById.get(hostId) || (host ? viewerRoomKeyForPart(host) : ''), roomKey);
  }) : [];
  splashes.forEach((s) => {
    const hostId = String(s && s.hostPartId);
...
function viewerActiveRoomLabel(snapshot){
  ...
  const groups = viewerCounterRoomGroups(snapshot || SNAPSHOT);
...
function render2DLayout(snapshot){
  ...
  for (const [stoneKey, parts] of byStone.entries()) {
    ...
    for (let i = 0; i < slabCount; i++) {
      ...
      ctx.strokeStyle = viewerIsLightTheme ? '#7f1d1d' : '#c78100';
      ctx.lineWidth = 2;
...
function drawAssembledItem2D(ctx, item, stonesByKey, opts){
  ...
  const rot = normDeg(placement.rot || 0);
  const palette = opts.palette || viewer2DPalette();
  ...
  const stroke = item.type === 'backsplash' ? palette.backsplashStroke : (item.type === 'waterfall' ? palette.waterfallStroke : palette.countertopStroke);
  ...
  if (item.type === 'countertop') {
    drawAssembledCutouts2D(ctx, item.cutouts, scale, palette);
    drawAssembledSeams2D(ctx, item.seams, scale, palette);
  }
  drawCenteredPieceLabel2D(ctx, geom, item.label || 'shape', scale, item.type, palette);
  ctx.restore();
}
...
function draw2DRoomSectionLabels(ctx, sections, opts){
  ...
  sections.forEach((section) => {
    ...
    ctx.fill();
    ctx.strokeStyle = palette.roomBadgeStroke;
    ctx.lineWidth = 1;
    ctx.stroke();
...
function drawCenteredPieceLabel2D(ctx, geom, label, scale, type, palette){
  ...
  const colors = palette || viewer2DPalette();
  ctx.save();
  ...
  ctx.fillStyle = type === 'countertop' ? colors.labelFillCounter : colors.labelFillOther;
...
function drawAssembledDimensions2D(ctx, item, opts){
  ...
  edges.forEach((edge) => {
    ...
    ctx.save();
    ctx.strokeStyle = type === 'countertop' ? palette.dimensionLineCounter : palette.dimensionLineOther;
    ctx.fillStyle = palette.labelFillCounter;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ...
    ctx.stroke();
    ctx.fillStyle = palette.dimensionBoxBg;
    ctx.strokeStyle = palette.dimensionBoxStroke;
    ctx.lineWidth = 1;
    ...
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = type === 'countertop' ? palette.dimensionTextCounter : palette.dimensionTextOther;
...
function drawAssembledCutouts2D(ctx, cutouts, scale, palette){
  ...
  ctx.save();
  ctx.strokeStyle = colors.cutoutStroke;
  ctx.lineWidth = 1.25;
  ctx.setLineDash([6, 4]);
  rows.forEach((c) => {
...
function drawAssembledSeams2D(ctx, seams, scale, palette){
  const rows = Array.isArray(seams) ? seams : [];
  if (!rows.length) return;
  const colors = palette || viewer2DPalette();
  ctx.save();
  ctx.strokeStyle = colors.seamStroke;
  ctx.lineWidth = 2;
...
function drawPart2D(ctx, part, pad, scale){
  ...
  const palette = viewer2DPalette();
  const partColor = part.partType === 'backsplash' ? palette.backsplashStroke : (part.partType === 'waterfall' ? palette.waterfallStroke : palette.countertopStroke);
  ...
  if (cutouts.length) {
    ...
    ctx.save();
    ctx.globalAlpha = 1;
    ctx.strokeStyle = palette.cutoutStroke;
    ctx.lineWidth = 1;
    ctx.setLineDash([6, 4]);
    cutouts.forEach((c) => {
      if (rotatedRoundRectPath2D(ctx, c, scale)) ctx.stroke();
  ...
  if (seams.length) {
    ctx.save();
    ctx.globalAlpha = 1;
    ctx.strokeStyle = palette.seamStroke;
    ctx.lineWidth = 2;
    ctx.setLineDash([10, 6]);
    seams.forEach((s) => {
      ctx.beginPath();
  ...
  drawCenteredPieceLabel2D(ctx, g, part.label || 'shape', scale, part.partType || 'countertop', palette);
  ctx.restore();
}
