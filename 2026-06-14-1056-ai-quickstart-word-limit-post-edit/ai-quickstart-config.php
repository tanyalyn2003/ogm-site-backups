...
{
    if (!empty($room['splashPieces'])) return;
    $labelN = ogm_qs_norm_label((string)($room['label'] ?? ''));
    ...
    if (preg_match('/\b(?:vanity|bathroom).{0,60}(?:backsplash|side splash)/i', $fullText)) {
        $scan = trim($scan . ' ' . $fullText);
    }
    if (preg_match('/no backsplash/i', $scan)) return;
    if (!preg_match('/\b(?:backsplash|side splash|full splash)\b/i', $scan)) return;

    $hint = 'Standard 4"';
    if (preg_match('/\bfull\s+backsplash\b/i', $segmentText)) {
        $hint = 'Full Splash';
    } elseif (preg_match('/\bfull\s+backsplash\b/i', $scan)) {
        $hint = 'Full Splash';
    } elseif (preg_match('/\bside\s+splash/i', $scan)) {
        $hint = 'Standard 4"';
    }
    if (empty($room['splashSuggestion'])) {
        $room['splashSuggestion'] = $hint;
    }
    if (($room['splashConfidence'] ?? 'low') === 'low') {
        $room['splashConfidence'] = 'medium';
    }
}
