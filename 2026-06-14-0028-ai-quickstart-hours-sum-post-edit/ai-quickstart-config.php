...
function ogm_qs_build_room_markers(string $text): array
{
    ...
    if (preg_match('/\b(?:kitchen|perimeter|L[\-\s]?shaped)\b/i', $text, $m, PREG_OFFSET_CAPTURE)) {
...
function ogm_qs_parse_text_measurements(string $description): array
{
    ...
    $kitchenSeg = ogm_qs_text_segment_for_label($text, 'Kitchen');
    $kitchenKey = ogm_qs_ensure_text_room($rooms, 'Kitchen');
    foreach (ogm_qs_extract_kitchen_runs($kitchenSeg) as $piece) {
    ...
    foreach (['Master Bath', 'Bathroom 2', 'Bath'] as $bathLabel) {
        $seg = ogm_qs_text_segment_for_label($text, $bathLabel);
        if ($seg === $text && $bathLabel !== 'Master Bath') {
            continue;
        }
...
function ogm_qs_match_text_room(array $textRooms, string $label): ?array
{
    ...
    if (str_contains($want, 'bath')) {
        $wantSecond = str_contains($want, '2') || str_contains($want, 'second');
        foreach ($textRooms as $tr) {
            $tl = ogm_qs_norm_label((string)($tr['label'] ?? ''));
            if (!str_contains($tl, 'bath')) continue;
            $trSecond = str_contains($tl, '2') || str_contains($tl, 'second');
...
function ogm_qs_apply_text_sink(array &$room, string $description, string $roomLabel = ''): void
{
    ...
    if (preg_match('/\b50[\s\-]*50[\s\-]*intrepid\b/i', $description)
        || preg_match('/\bintrepid[\s\-]*50[\s\-]*50\b/i', $description)) {
